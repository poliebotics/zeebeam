//! Exact local CPU core prover and cold verifier for the frozen membership guest.
...
fn usage() -> &'static str {
    "usage:\n  zeebeam-row-binding-membership-core prove --raw PATH --output PATH\n  zeebeam-row-binding-membership-core verify --proof PATH --expected-bytes N --expected-sha256 HEX"
}
...
fn verify_original_and_tamper<P: Prover>(
    ...
    let original = proof.public_values.clone();
    let mut changed = original.to_vec();
    changed[TAMPER_PUBLIC_BYTE] ^= 1;
    proof.public_values = SP1PublicValues::from(&changed);
    let rejection = client.verify(proof, vkey, None);
    proof.public_values = original;
    match rejection {
        Err(SP1VerificationError::InvalidPublicValues) => Ok(()),
        Err(error) => Err(format!(
            "single-bit public mutation produced an unexpected verifier error: {error}"
        )),
        Ok(()) => Err("single-bit public mutation was accepted".to_string()),
    }
}
...
fn prove(raw: &Path, output: &Path) -> Result<(), String> {
    ...
    println!("sp1_core_opts=pinned_6.4.0_defaults");
    ...
    let mut proof = client
        ...
        .run()
        .map_err(|error| format!("SP1 core proof failed: {error}"))?;
    let prove_ms = prove_started.elapsed().as_millis();
    if !matches!(&proof.proof, SP1Proof::Core(_)) {
        return Err("SP1 returned a non-core proof".to_string());
    }
...
fn verify(path: &Path, expected_bytes: u64, expected_sha256: &str) -> Result<(), String> {
    ...
    if !matches!(&proof.proof, SP1Proof::Core(_)) {
        return Err("loaded proof is not core mode".to_string());
    }
    let verify_started = Instant::now();
    verify_original_and_tamper(&client, &mut proof, proving_key.verifying_key(), &expected)?;
    println!("classification={CLASSIFICATION}");
    println!("mode=cold_verify_core");
    println!("ambient_sp1_controls=absent");
    println!("sp1_core_opts=pinned_6.4.0_defaults");
    println!("guest_elf_bytes={ELF_BYTES}");
    println!("guest_elf_sha256={ELF_SHA256}");
...
mod tests {
    ...
    fn exact_prove_grammar() {
        assert_eq!(
            ...
            Ok(Command::Prove {
                ...
                output: PathBuf::from("/output/core.bin"),
            })
        );
    }
    ...
    fn exact_verify_grammar() {
        assert_eq!(
            ...
            Ok(Command::Verify {
                proof: PathBuf::from("/output/core.bin"),
                expected_bytes: 123,
