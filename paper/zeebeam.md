...
version: 3.22
date: 2026-09-23
status: manuscript-v3.22-realness-records-recovered
...
training seed is unrecorded, and the configured seeds 11, 22 and 33 were never trained. A generator-free replay-and-splice
diagnostic (class 3: real frames presented at chain positions they do not belong to) ran nine times, three evaluation
seeds by three attack seeds, on 6 real against 18 attacked subsequences per run; every run had zero attacks passing,
zero reals rejected and AUROC 1.0. It is a security diagnostic, not a realness ROC, and is not pooled with the
generated-fakes result. The project note also records a black-box ℓ2 probe on the rented box; its run record was not
preserved when the box was terminated, so no figure from it is reported here (`ml/realness/README.md`). The adaptive white-box attack (class 4) is **outstanding** (it
...
configuration. The exact generating script recorded by the run (`fdb8745d…`) and the class-3 record were recovered
on 23 September 2026 from the project's backup of the rented box's filesystem and are in `ml/realness/recovered/`;
with the scorer checkpoint (`d074775e…`) published, both runs can be re-executed, and every run's AUROC recomputes
from the scores in its record (`ml/realness/README.md`). These are diagnostics on a
...
ROC record, the harness code and configuration, and under `recovered/` its generating script, the replay/splice
record and the fake-generation manifests; the ℓ2 run record was not preserved and Section 8.1 says so), the 257 fleet proofs of the remaining anchored rows with their manifests, raw
...
- 3.22 (2026-09-23, BOSUN) — Section 8.1 and the artifact list: the generated-fakes run's generating script and the class-3 replay-and-splice record, previously reported as lost with the rented box, were recovered from the project's backup of its filesystem into `ml/realness/recovered/`; the class-3 figures are reported (nine runs, zero attacks passing, zero reals rejected, AUROC 1.0, a security diagnostic not pooled with the generated-fakes result); the scorer checkpoint is published; the ℓ2 record remains unpreserved; HTML and PDF re-rendered; no other change.
